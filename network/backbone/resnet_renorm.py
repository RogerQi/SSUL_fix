...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dilation=1, conv=None, norm=None, return_att=False):
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        for i in range(1, blocks):
            if i == blocks - 1:
                layers.append(block(self.inplanes, planes, dilation=dilation, conv=self.conv, norm=self.norm, return_att=True))
            else:
    ...
    def forward(self, x, return_intermediate=False):
        ...
        # Save output from every residual layer
        intermediate_outputs = []

        x, att = self.layer1(x)
        intermediate_outputs.append(att)
...
def deeplabv3_resnet101_renorm(pretrained=True):
    ...
    if pretrained:
        # TODO: specify pretrained path?
        pretrained_weight_dict = torch.load('checkpoints/resnet101-5d3b4d8f.pth')
        print(model.load_state_dict(pretrained_weight_dict, strict=False))
    return model
